...
knitr::opts_knit$set(root.dir = '/visualization')
...
```{r}
ath<-read.csv("data/athletes_and_events.csv")
gdp <- read.csv("data/gdp_pop.csv")
...
##### (i)
```{r}
# count Winter Olympic medals by country by year
medals_count_year <- total_df[complete.cases(total_df[ , 15]),] %>%
  filter(Season == 'Winter') %>%
  count(NOC, Year, sort = TRUE)

# order descending by country
medals_count_year <- medals_count_year[order(medals_count_year$n, decreasing=TRUE),]

# filter to the top 10 countries
medals_count_year_top10 <- medals_count_year %>%
  filter(NOC %in% head(medals_count_year, 10)$NOC)
...
```{r}
# filter on winter games
is.winter <- total_df[total_df$Season=='Winter',]

# create table of winter games by country
...
plot_gold<-head(topcountry.gold,25) %>%
  ...
  scale_fill_manual(values=c("USA" = "dodgerblue2",
                             "CAN"= "brown1"))+
  theme_bw()+
...
plot_bronze<-head(topcountry.bronze,25) %>%
  ...
  theme(legend.position="left", 
        legend.title = element_text(size = 8),
        legend.text = element_text(size = 8))+
  labs(x="Name Of Countries", y = "Medals Count",
...
# aggregate all the plots together
...
```{r}
# create data frames by athlete
ath <- merge(ath, noc, by.x = "NOC", by.y = "NOC")
ath <- merge(ath, gdp, by.x = "NOC", by.y = "Code")

# filter on Winter Olympic athletes
medals_count <- ath[complete.cases(ath[ , 15]),] %>%
  filter(Season == 'Winter') %>%
  count(NOC, sort = TRUE)

# count athlete medals
...
#create plot of medals by GDP
medal_gdp_plot <-medal_gdp_df_1 %>%
  ggplot(aes(x = reorder(NOC, gdp_new),
             y = gdp_new,
             fill = NOC))+
  geom_bar(stat ="identity")+
  scale_fill_discrete("NOC")+
  ...
  labs(x="Name Of Countries",
       y="Medals Count",
       title="Number Of Medals Won Adjusted By Per.Capita.GDP")+
...
# create "medals adjusted by gdp" plot
medal_gdp_plot <-medal_gdp_df_2 %>%
  ggplot(aes(x = reorder(NOC, pop_new),
             y = pop_new,
             fill = NOC))+
  geom_bar(stat = "identity")+
  scale_fill_discrete("NOC")+
  ...
  labs(x = "Name Of Countries",
       y = "Medals Count",
       title = "Number Of Medals Won Adjusted By Population")+
  theme(plot.title = element_text(hjust = 0.5, size = 15))+
  theme(axis.text.x = element_text(size = 9),
        axis.text.y = element_text(size = 8),
        axis.title.x = element_text(size = 10),
...
```{r}
# filter on non-null host city entries
host_cities <- hosts %>% filter(Winter != "") %>%
  select(City, Country, Year)

host_cities$Country[host_cities$Country ==	'Russia[h]'] <- 'Russia'

home_adv_diff <- numeric()

# count number of medals in the host year compared to the previous year's medals
for(row in 1:nrow(host_cities)){
  if(row != 1){
    year = host_cities[[row, 'Year']]
    country = host_cities[[row, 'Country']]
    host_medal = ath[complete.cases(ath[ , 15]),] %>%
      filter(Country == country,
             Season == 'Winter',
             Year == year) %>%
      nrow()
    previous_medal = ath[complete.cases(ath[ , 15]),] %>%
      filter(Country == country,
             Season == 'Winter',
             Year == year-4) %>%
      nrow()
    ...
    home_adv_diff[row] = host_medal-previous_medal
    } else{
      home_adv_diff[row] <- 0
    }
  }

# create data frame that aggregates the medal differences by country
host_diff <- data.frame(country = host_cities$Country, home_adv_diff)

# create plot
ggplot(host_diff, aes(x = reorder(country, diff), y = home_adv_diff, fill = country)) +   
...
```{r}
# filter athlete medals to Gold Winter Olympic medals
# extract certain athletes' attributes
...
```{r}
# filter to the top 15 players
top_15_player <- head(player_medals_count, 15)

# plot the top 15 players
plot_top_15_player <- ggplot(top_15_player,
...
```{r}
# create violin plot of athletes' weights
weight_plot <- ggplot(head(player_medals_count,100),
...
# create violin plot of athletes' heights
height_plot <- ggplot(head(player_medals_count,100),
                      aes(x=Sex,
                          y=Height,
                          fill=Sex)) + 
 geom_violin() +
...
```{r}
# create interactive plot of top 15 players
plot_ly(data = top_15_player, 
        x = ~reorder(Name, desc(n)),
        y = ~n,
        color = ~Name, 
        type = "bar") %>% 
  layout(title = "Most Successful Athletes",
         yaxis = list(title = "Medals"),
         xaxis = list(title = "Athelete Name"))
```
...
```{r}
# create interactive data table of medals by country GDP
library(DT)
datatable(medal_gdp_df, colnames = c("Country Code",
                                     "Number of Medals",
                                     "Country Name",
                                     "Population",
                                     "Per Capita GDP"))
